"""
Offline simulator for the HSpec "optim" strategy.

This script reads trainer-side HSPEC_DUMP artifacts and simulates, for each
epoch/prompt/rollout, the idealised HSpec policy that:

1. Looks at *all* entries in the active table used by that epoch.
2. Chooses the entry whose draft continuation has the largest exact-match
   length against the known rollout continuation.
3. Chooses the draft length to be exactly that maximal match length.

The simulation is fully offline: no model execution happens here.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Offline simulation of the optim HSpec strategy.",
    )
    parser.add_argument(
        "--dump-root",
        type=Path,
        default="/workspace/exp/hspec_dump",
        required=True,
...
def simulate_rollout_optim(
    ...
    while pointer < seq_len - 1:
        ...
        for entry_id in range(table.n_entries):
            draft = entry_drafts[entry_id]
            match_len = longest_exact_match(draft, continuation)
            if match_len <= 0:
                continue
            similarity = float(np.dot(query_key, table.keys[entry_id]))
            if match_len > best_match_len:
                best_match_len = int(match_len)
                ...
                if best_similarity is None or similarity > best_similarity:
                    best_entry_id = entry_id
                    best_similarity = similarity
                elif similarity == best_similarity and entry_id < best_entry_id:
                    best_entry_id = entry_id

        if best_match_len <= 0 or best_entry_id < 0:
        ...
        selected_accept_lengths.append(best_match_len)
        selected_draft_lengths.append(best_match_len)
        selected_similarities.append(float(best_similarity))
        ...
        tie_counts.append(best_tie_count)
        pointer = pointer + best_match_len
...
def write_rollout_result(
    ...
    payload: dict[str, Any] = {
        ...
        "total_propose_steps": np.asarray(
            sum(int(r["total_propose_steps"]) for r in rollout_results), dtype=np.int32
        ),
        "match_rate": np.asarray(
            (
                sum(int(r["num_match_steps"]) for r in rollout_results)
                / sum(int(r["total_propose_steps"]) for r in rollout_results)
            )
            if sum(int(r["total_propose_steps"]) for r in rollout_results) > 0
            else 0.0,
            dtype=np.float32,
        ),
...
def simulate_epoch(
    ...
    decisions_dir = epoch_out_dir / "optim_decisions"
    decisions_dir.mkdir(parents=True, exist_ok=True)
    step_summaries_dir = epoch_out_dir / "step_summaries_optim"
...
def main() -> None:
    ...
    for epoch, epoch_dir in iter_epoch_dirs(dump_root, args.epoch_start):
        ...
        print(
            f"[optim] epoch={epoch} prompts={summary['prompts_simulated']} "
