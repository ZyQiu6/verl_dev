...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        from verl.trainer.main_ppo_offpolicy import (create_rl_dataset,
                                                     create_rl_sampler)
        ...
        total_training_steps = len(self.train_dataloader) * self.config.trainer.total_epochs * self.config.trainer.offpolicy_sync_freq
    ...
    def _load_checkpoint(self):
        ...
        self.rollout_wg.load_checkpoint(actor_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        # load critic
        if self.use_critic:
    ...
    def gen_and_store_rollout(self, batch: DataProto, gen_batch: DataProto, partial_rollout_enable=False,
        ...
        if switch_role:
            ...
            if not self.async_rollout_mode:
                ...
                # self.async_rollout_manager.wake_up()
                if len(batch) > 0 and len(gen_batch) > 0:
        ...
        self.lock.acquire()
        self.gen_buffer.add(batch, info)
        self.lock.release()
        
        print(f"Gen store_buffer length: {len(self.gen_buffer)}")
        return batch

    def inference(self, batch, extra_info={}) -> DataProto:
        timing_raw = {}
        metrics = {}
        ...
        if "version" in extra_info:
            ...
            print(f"Generation version {version}")
        self.lock.acquire()
        self.inference_buffer.add(batch, info)
        self.lock.release()
        
        if self.use_critic:
            # if self.inference_train_batch:
            #     DataProto.concat([self.inference_train_batch, batch])
            # else:
            #     self.inference_train_batch = deepcopy(batch)
            with _timer("update_critic", timing_raw):
                critic_output = self.critic_wg.update_critic(batch)
            critic_output_metrics = reduce_metrics(critic_output.meta_info["metrics"])
            metrics.update(critic_output_metrics)
            
        print(f"timing_s: {timing_raw}")
    ...
    async def update(self, timing_raw, metrics, config={}, stop_flag=False) -> DataProto:
        ...
        while len(batch) < train_batch_size:
            self.lock.acquire()
            batch, _ = self.inference_buffer.select(train_batch_size, config)
            self.lock.release()
    ...
    def fit(self):
        ...
        self.inference_train_batch = None
