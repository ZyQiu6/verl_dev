...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # max_num_seqs=config.max_num_seqs,
        # max_num_batched_tokens=max_num_batched_tokens,
        print("vllm can see tensor_parallel_size:", tensor_parallel_size)
        #####修改动态切换
        # --- 新增：tail 模式 flag ---
        ...
        self.inference_engine = self.engine_ep
    ...
    def set_tail_mode(self, in_tail: bool):
        """外部调用：切换是否进入 tail 模式（= 切换引擎）"""
        if in_tail == self._in_tail_mode:
            return  # 状态没变就别折腾

        self._in_tail_mode = in_tail
        if in_tail:
            # 小 batch / 长尾：用 TP-only 引擎
            self.inference_engine = self.engine_tp
        else:
            # 正常：用 EP 引擎
            self.inference_engine = self.engine_ep
        print("successfully change parallel mode")
    ...
    async def update_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None], **kwargs):
        ...
        if peft_config and base_sync_done:
            ...
            model1 = self.engine_ep.llm_engine.model_executor.driver_worker.worker.model_runner.model
            patch_vllm_moe_model_weight_loader(model1)
            model1.load_weights(weights)
            model2 = self.engine_tp.llm_engine.model_executor.driver_worker.worker.model_runner.model
            patch_vllm_moe_model_weight_loader(model2)
