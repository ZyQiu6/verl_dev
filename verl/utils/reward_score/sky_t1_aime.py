...
from .math_util import math_normalize
from .math_util.grader import math_equal
from .math_util.testing_utlis import math_equal as math_equal_2
from .math_util.testing_utlis import strip_answer_string, extract_answer
...
def evaluate_math(model_output: str, ground_truth: str, do_print=False):
    model_output = str(model_output)
    ground_truth = str(ground_truth)

    is_matched, extracted_model_output = match_answer(model_output)
    # if not is_matched:
    #     extracted_model_output = extract_answer(model_output)
    format_correctness = "<|end_of_solution|>" in model_output and "\\box" in model_output
    if do_print:
    ...
    # if not is_correct:
    #     if math_equal_2(extracted_model_output, strip_answer_string(ground_truth)):
...
def compute_score(solution_str, ground_truth, format_score=0.1, score=1.0):
    is_correct, format_correctness, extracted_model_output = evaluate_math(model_output=solution_str, ground_truth=ground_truth)
    if format_correctness:
        if is_correct:
            return score
        else:
            return format_score
    ...
    # if there is any need to use reward model
    # reward_tensor_dict={}
    # reward_metrics={}
